...
-c <path to controls>             Specify path to control files with Genus_species_*control.gff names
...
while getopts 'ac:d:e:f:hi:m:o:p:rs:t:' OPTION
do
  case "$OPTION" in
    a)
...
do
  ...
  then
    if [ -z "$expand_flag" ]
    then
      echo "removing $directory due to only containing $number_of_files"
...
mkdir -p $out/$run_date/Salmonella/enterica/all
control_list=($(ls $control_path/*_control.gff))
...
#  do
#    control_parts=($(echo "$control" | rev | cut -f 1 -d "/" | rev | cut -f 1,2 -d "_" ))
#    echo $control_parts
